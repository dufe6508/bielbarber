  ...
  "group/button inline-flex shrink-0 items-center justify-center rounded-lg border border-transparent bg-clip-padding text-sm font-medium whitespace-nowrap transition-all outline-none select-none focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50 active:not-aria-[haspopup]:scale-[0.97] disabled:pointer-events-none disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-3 aria-invalid:ring-destructive/20 dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
  {
    variants: {
      variant: {
        ...
        destructive:
          "bg-danger-muted text-danger-muted-foreground hover:bg-danger-muted/80 focus-visible:border-danger/40 focus-visible:ring-danger/20 dark:bg-danger-muted dark:hover:bg-danger-muted/80 dark:focus-visible:ring-danger/40",
        link: "text-primary underline-offset-4 hover:underline",
      },
      size: {
        /* xs  — 24px — apenas contextos muito compactos               */
        ...
        /* sm  — 32px — ações secundárias, within-card                 */
        ...
        /* default — 40px — botões padrão (tabs, actions, ghost)       */
        default: "h-10 gap-1.5 px-3.5 has-data-[icon=inline-end]:pr-2.5 has-data-[icon=inline-start]:pl-2.5",
        /* lg  — 44px — CTAs destacados, barra de ação                 */
        lg: "h-11 gap-2 px-5 text-sm font-semibold has-data-[icon=inline-end]:pr-3.5 has-data-[icon=inline-start]:pl-3.5",
        /* xl  — 48px — CTA principal (confirmar, concluir)            */
        xl: "h-12 gap-2 px-6 text-sm font-semibold has-data-[icon=inline-end]:pr-4 has-data-[icon=inline-start]:pl-4",
        /* ícones */
        icon:    "size-10",
        "icon-xs": "size-6 rounded-[min(var(--radius-md),10px)] [&_svg:not([class*='size-'])]:size-3",
        "icon-sm": "size-8 rounded-[min(var(--radius-md),12px)] [&_svg:not([class*='size-'])]:size-3.5",
        "icon-lg": "size-11",
        "icon-xl": "size-12",
      },
    },
